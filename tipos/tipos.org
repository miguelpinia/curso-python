...
permiten que estas sean utilizadas como pilas. La
operación de las pilas se puede decir de forma sencilla
que es una estructura LIFO (last-in first-out). Para
agregar un elemento al principio de la lista, se usa
*append()*, mientras que para extraer, usamos *pop()* sin
un índice explicito.
