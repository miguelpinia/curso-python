...
- Asumo cierta experiencia en la programación
- Asumo que han usado de forma básica Emacs.
- No voy a cubrir todas las API's sólo van ser cuestiones
  de sintaxis y formas introductorias a las herramientas.
...
- Padre :: Python nació de la mano de *Guido van Rossum*,
     ...
     Informática de los Países Bajos. Buscaba un
...
- Tomar ideas prestadas de otros lugares siempre que
  tuviera sentido.
- “Las cosas deben ser tan simples como sea posible,
  pero no más simples.” (Einstein)
- Haz una cosa bien (la “filosofía de Unix”).
- No preocuparse mucho sobre el performance; planea
  primero, optimizar luego, siempre y cuando sea
  necesario.
- No pelear con el entorno y seguir la corriente.
- No buscar la perfección porqué “suficientemente
  bueno” es a menudo eso (suficientemente bueno).
...
- Los errores no deben ser fatales. Esto significa que
  el código de los usuarios debe ser capaz de
  recuperarse de condiciones de error mientras que la
  máquina virtual siga funcionando.

- Al mismo tiempo, los errores no deben pasar
  inadvertidos (estos dos últimos elementos llevan
